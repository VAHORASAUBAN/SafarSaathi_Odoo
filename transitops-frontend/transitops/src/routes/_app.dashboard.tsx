...
import { mapApiVehiclesToFrontend, mapApiDriversToFrontend, mapApiTripsToFrontend } from "@/lib/mappers";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 xl:grid-cols-7">
        ...
        <Kpi icon={Truck} label="Active Trips" value={kpis?.active_trips ?? activeTrips} tone="text-info" />
        <Kpi icon={Clock} label="Pending Trips" value={kpis?.pending_trips ?? pendingTrips} />
        <Kpi icon={UserCheck} label="Drivers On Duty" value={kpis?.drivers_on_duty ?? onDuty} tone="text-success" />
